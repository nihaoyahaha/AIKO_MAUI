...
//点文本图片
public class CCheckPoint : ICustomShape
{
    #region 属性
    private string _parentRectId;
    public virtual string ParentRectId { get => _parentRectId; set => _parentRectId = value; }
    ...
    private CustomShapeType _type = CustomShapeType.PointTextImage;
    ...
    public bool IsTextVisible { get => _isTextVisible; set => _isTextVisible = value; }
    ...
    public CCheckPoint(Microsoft.Maui.Graphics.IImage source, string parentRectId)
    {
        _source = source;
        _parentRectId = parentRectId;
    }
    ...
    {
        // if (new System.Drawing.RectangleF(_x, _y, _source.Width, _source.Height).Contains(new System.Drawing.PointF(pt.X,pt.Y)))
    ...
    private void SetRectBorderSize(SizeF stringSize)
    {
        if (_isTextVisible)
        {
            if (!string.IsNullOrEmpty(_text))
                _rectBorder = new System.Drawing.RectangleF(_x - 3, _y - 3, _source.Width + stringSize.Width, stringSize.Height);
            else
    ...
    public void Draw(ICanvas canvas, RectF dirtyRect)
    {
        ...
        SizeF size = string.IsNullOrEmpty(_text) ? new SizeF() : canvas.GetStringSize(_text, _font, _fontSize, HorizontalAlignment.Left, VerticalAlignment.Center);
    ...
    {
        if (_focuseType != CustomShapeFocusType.Unfocused)
        {
            _selected = true;
        }
        else
